...
// Witnesses the polytopal requirement: 2-cells of different arity in one
// complex, sharing a derived 1-cell.
...
GRAPHOS_TEST(triangle_pair_matches_hand_built_counts) {
  ...
  CHECK(c.count(2) == 2);
  CHECK(graphos::d_squared_is_zero(c));
  // opposite windings across the shared 1-cell: coherent as built
...
GRAPHOS_TEST(two_cubes_share_a_derived_face) {
  ...
  CHECK(graphos::d_squared_is_zero(c));
  CHECK(graphos::euler_characteristic(c) == 1);
  // outward windings on both cells: coherent as built
  const auto o = graphos::orient(c);
  CHECK(o.orientable);
...
// Witnesses derivation in dimension 4: one 4-simplex, every stratum derived,
// with counts C(5, k+1).
...
// Witnesses ∂Δ⁴ ≅ S³: extracted by the generic operations, with the Betti
// numbers of the 3-sphere.
...
// Witnesses coherence in dimension 4: two 4-simplices sharing a facet, given
// in agreeing orientations.
...
GRAPHOS_TEST(rejects_malformed_input) {
  ...
  // one vertex set, cyclically incompatible orderings
